...
public class YNTRUSTUploadAttachment extends UploadAttachmentDomain<OsbAttachmentDTO>{
    ...
    public void upLoadTask(String compressTranNo){
        try {
            ...
            logger.info("云信订单号{},阶段{}上传文件的返回数据为{}",osbFileLog.getBzId()
    ...
    public void upLoadTask(String bzId, String bzType,
        ...
        //调用压缩服务
        logger.info("申请编号{},调用压缩服务的请求数据为{}",bzId,result);
	...
	private void dealOsbFileLog(OsbFileLog osbFileLog, BaseMsgDTO baseMsgDTO) {
        ...
        if (Objects.equals(osbFileLog.getBzType(),
            ...
            if (mainInfo != null){
                mainInfo.setAfterLoanState(getStatusByBaseMsg(baseMsgDTO));
                mainInfo.update();
            }
        }
    }

    private Integer getStatusByBaseMsg(BaseMsgDTO baseMsgDTO) {
        if (baseMsgDTO.successed()){
            return AssetStateEnum.SUCCESS.getState();
        }
        if (Objects.equals(CommonConstant.FAILURE,baseMsgDTO.getCode())){
            return AssetStateEnum.FAILD.getState();
        }
        if (Objects.equals(CommonConstant.PROCESSING,baseMsgDTO.getCode())){
            return AssetStateEnum.DOING.getState();
        }
        return AssetStateEnum.INIT.getState();
    }
    ...
    public InvokeResult<DscSupplyDto> checkAttachmentFile(OsbAttachmentDTO osbAttachmentDTO) {
        ...
        //mortgageRegistrationCertificate --- mortgageAcceptanceCertificate
        //请款阶段抵押登记证和抵押受理证明特殊处理
        if (Objects.equals(osbAttachmentDTO.getBzType(),BankPhaseEnum.PAYMENT.getPhase())){

            AssetAttachmentRule registration = rules.get("mortgageRegistrationCertificate");
            AssetAttachmentRule acceptance = rules.get("mortgageAcceptanceCertificate");


            if(CollectionUtils.isEmpty(attachmentMap.get(registration.getAttachMainType()))
                    && CollectionUtils.isEmpty(attachmentMap.get(acceptance.getAttachMainType()))){

                dscSupplyDto = new DscSupplyDto();
                results = new ArrayList<>(2);


                DscSupplyAttachmentsDto attachmentsDto1 = new DscSupplyAttachmentsDto(1,-1);
                attachmentsDto1.setAttName(registration.getAttachName());
                attachmentsDto1.setAttType(
                        StringUtils.isEmpty(registration.getMergerAttachType())
                                ? registration.getAttachMainType():registration.getMergerAttachType());

                DscSupplyAttachmentsDto attachmentsDto2 = new DscSupplyAttachmentsDto(1,-1);
                attachmentsDto2.setAttName(acceptance.getAttachName());
                attachmentsDto2.setAttType(
                        StringUtils.isEmpty(acceptance.getMergerAttachType())
                                ? acceptance.getAttachMainType():acceptance.getMergerAttachType());

                results.add(attachmentsDto1);
                results.add(attachmentsDto2);
                dscSupplyDto.setAttList(results);
                invokeResult.failure("缺少必要附件类型").setData(dscSupplyDto);
                return invokeResult;
            }
