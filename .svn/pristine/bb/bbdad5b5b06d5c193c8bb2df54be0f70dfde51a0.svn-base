...
import com.yixin.web.dto.monitor.BzErrorInfoDto;
...
public class BzErrorNoticeRunnable implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            //记录业务异常信息到mongo
            BzErrorInfoDto bzErrorInfo = new BzErrorInfoDto();
            bzErrorInfo.setBzId(bzId);
            bzErrorInfo.setInterfaceName(interfaceName);
            bzErrorInfo.setMessage(message);
            bzErrorInfo.seteStackTrace(StrUtil.getStackTrace(e));
            mongoTemplate.save(bzErrorInfo);
