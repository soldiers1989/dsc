...
public class PaymentRequestStrategy extends CommonRequest4Icbc<ICBCRequestFundsDTO> {
	...
	protected void getData() throws BzException {
		...
		if (BigDecimal.ZERO.compareTo(txamt) == 0) {
			//补息标识
			fundsDTO.setTxflag(IcbcConstant.TXFLAG_0);
		}else {
			//补息标识
			fundsDTO.setTxflag(IcbcConstant.TXFLAG_1);
			fundsDTO.setTxamt(txamt.toString());
		}
	...
	protected void setTxtCode() {
		...
		if(assetMainInfo == null){
			logger.info("申请单号为{}，订单还未发起请款", this.inputDto.get().toString());
			throw new BzException("订单还未发起请款");
		}else{
			//判断是是否是驳回
			if(assetMainInfo.getAssetPhase().equals(BankPhaseEnum.PAYMENT_REJECT.getPhase())){
				this.dataThreadLocal.get().getComm().setTrxcode(IcbcConstant.TRXCODE_40181);//请款驳回   
			} else {
				this.dataThreadLocal.get().getComm().setTrxcode(IcbcConstant.TRXCODE_40101);//请款
			}
		}
		logger.info("[工商银行]请款设置交易码值applyNo：{}，trxCode：{}", this.inputDto.get(), this.dataThreadLocal.get().getComm().getTrxcode());
	}
	...
	private void assembleCar(ICBCRequestFundsDTO fundsDTO, DscSalesApplyCar carDTO, DscSalesApplyPayee applyPayee) {
		...
		fundsDTO.setBuydate(carDTO.getBuyCarTime().toString().replaceAll("-","").substring(0,6));//购车年月
