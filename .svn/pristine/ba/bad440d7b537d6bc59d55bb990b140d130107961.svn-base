         ...
         */
	      ...
	      if (response.getCode().contains(CommonConstant.WB_RESP_CODE)) {
	    	  ...
	    	  if(notifyAlix){
	    		  ...
	    		  ioc.publishEvent(new ResultNotifyEvent(this.createForAliDto(assetMainInfo.getApplyNo(), CreditfrontResultEnum.SUCCESS,CreditfrontResultEnum.SUCCESS.getMsg())));
	    	  }
	      ...
	      if (response.getCode().contains(CommonConstant.WeBankErrorCode.VIN)){ //复审驳回 返回码1034，VIN码格式错误
	    	  if(notifyAlix){
	    	  ...
	    	  if(notifyAlix){
	    		  ...
	    		  ioc.publishEvent(new ResultNotifyEvent(this.createForAliDto(assetMainInfo.getApplyNo(), CreditfrontResultEnum.REFUSE,response.getDesc())));
	    	  }
	      }
        return new InvokeResult<>(new BaseMsgDTO(CommonConstant.FAILURE,response.getDesc()));
    }
