...
public class DscShuntImpl implements DscShuntService {
	...
	public DscAdmittanceReturnDto capitalAccessCheck(String applyNo, String capitalId,String capitalCode) {
		...
		String shuntId = DscOrderTrack.recordTrack(applyNo, OrderTrackTypeEnum.SHUNT,null,capitalCode);
		...
		StringBuilder affiliatedCapital = new StringBuilder();
		...
		if(StringUtils.isBlank(capitalId)){ //资方id为空
			...
			if(CollectionUtils.isEmpty(matchingCapitalList)){ //根据订单获取到的资方列表为空
				...
				return this.supplementAlixReturnData(ShuntRefuseTypeEnum.PRODUCT_NO_CAPITAL,shuntId,applyNo,null);
			}
			for(int i=0 ;i<matchingCapitalList.size();i++){
				DscCapitalDto dto = matchingCapitalList.get(i);
				if(dto!=null) {
					affiliatedCapital.append(dto.getCapitalCode());//资方CODE
					if(matchingCapitalList.size()-1>i) {
						affiliatedCapital.append(",");
					}
				}
			}
		...
		LOGGER.info("准入规则校验-过滤资方-开始。 订单编号:{}，过滤前资方列表:{}",applyNo, JSON.toJSONString(capitalList));
		capitalList = this.filterCapitalList(capitalList, applyNo);
		LOGGER.info("准入规则校验-过滤资方-结束。 订单编号:{}，过滤后资方列表:{}",applyNo, JSON.toJSONString(capitalList));
		...
		if(CollectionUtils.isEmpty(matchedCapitalList)){
			...
			return this.supplementAlixReturnData(ShuntRefuseTypeEnum.RULE_NO_MATCH,shuntId,applyNo,affiliatedCapital.toString());
		}
		...
		DscAdmittanceReturnDto result = this.supplementAlixReturnData(finalCapitalDto,applyNo,sourceMap,shuntId,affiliatedCapital.toString());
		...
		// 准入拒绝类型   
		result.setShuntRefuseType(ShuntRefuseTypeEnum.MATCH_SUCCESS.getType());
		return result;
	}
	...
	private void checkOrderCanShunt(String applyNo,String capitalId){
		...
		DscSalesApplyCust dscSalesApplyCust = DscSalesApplyCust.getApplyCostByMianId(applyMain.getId());
		if(dscSalesApplyCust==null){
			LOGGER.info("该订单没有合同申请客户数据，请先同步后准入,订单编号:{}",applyNo);
			throw new BzException("该订单没有合同申请客户数据，请先同步后准入");
		}
	...
	private DscAdmittanceReturnDto supplementAlixReturnData(DscCapitalDto finalCapitalDto,String applyNo,
			Map<String, Object> sourceMap,String shuntId,String affiliatedCapital){
		...
		DscOrderTrack.updateTrackSuccessResult(shuntId, JSON.toJSONString(finalCapitalDto),affiliatedCapital);
	 ...
	 */
	private DscAdmittanceReturnDto supplementAlixReturnData(ShuntRefuseTypeEnum refuseTypeEnum,String shuntId,String
			applyNo,String affiliatedCapital){
		...
		if(mainInfo != null){
			LOGGER.info("由于准入返回了YX所以清空资方流程表，applyNo:{}",applyNo);
			mainInfo.setDeleted(true);
			mainInfo.update();
		}
		DscOrderTrack.updateTrackSuccessResult(shuntId, JSON.toJSONString(result),affiliatedCapital);
		return result;
	}
	...
	public Map<String, Object> getOrderDataSource(String applyNo) {
		...
		//-------需要计算的服务费--------------
		DscComputeDTO dscCalculationDTO = settleService.computeServiceFee(sourceMap,applyNo);
		sourceMap.put(DscComputeDTO.class.getName(),dscCalculationDTO);
		return sourceMap;
	}
	/**
	 * 规则验证接口
