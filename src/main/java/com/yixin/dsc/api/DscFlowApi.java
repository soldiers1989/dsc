...
public class DscFlowApi {
     ...
     */
    @InterfaceMonitor(errorMsg = "取消、提回、退回订单请求接口异常", keyParam = {"applyNo::0_applyNo", "ctype::0_ctype"})
     ...
     */
    @InterfaceMonitor(errorMsg = "资方审核结果反推异常")
     ...
     */
    @InterfaceMonitor(errorMsg = "发起银行信审失败", keyParam = {"applyNo::0_applyNo"})
    ...
    public InvokeResult<DscActionResultDTO> sendBankCreditfront(@RequestBody DscActionParamDTO dscActionParamDTO) {
        ...
        String capitalCode = AssetFinanceInfo.getFinanceCodeById(capitalId);
        
        orderOperateService.recordOrderOperate(applyNo, OrderOperate.REC_ALIX_AUDIT_APPLY, null);

        String shuntId = DscOrderTrack.recordTrack(applyNo, OrderTrackTypeEnum.CREDITFRONT_SEND, null,capitalCode);
        ...
        LOGGER.info("发起银行信审,订单编号:{},记录申请单的操作开始", applyNo);

        //向银行发起信审
        orderOperateService.recordOrderOperate(applyNo, OrderOperate.WAIT_BANK_AUDIT, null);
        LOGGER.info("发起银行信审,订单编号:{},记录申请单的操作结束", applyNo);
        LOGGER.info("发起银行信审,订单编号:{},返参:{}", applyNo, JSON.toJSONString(result));
        return result;
    }
     ...
     * 2018年6月4日 上午10:10:32
     */
    @InterfaceMonitor(errorMsg = "发起银行请款失败", keyParam = {"applyNo::0_applyNo"})
     ...
     */
    @InterfaceMonitor(errorMsg = "未获取到授权书签署状态", keyParam = {"applyNo::0_applyNo"})
     ...
     */
    @InterfaceMonitor(errorMsg = "未获取到银行合同签署状态", keyParam = {"applyNo::0_applyNo"})
     ...
     */
    @InterfaceMonitor(errorMsg = "未获取到银行请款状态", keyParam = {"applyNo::0_applyNo"})
     ...
     */
    @InterfaceMonitor(errorMsg = "未获取订单信审状态", keyParam = {"applyNo::0_applyNo"})
     ...
     * 2018年7月4日 下午5:09:56
     */
    @InterfaceMonitor(errorMsg = "电子征信存证失败", keyParam = {"applyNo::0_applyNo"})
     ...
     * 2018年7月4日 下午5:09:56
     */
    @InterfaceMonitor(errorMsg = "电子合同签约存证失败", keyParam = {"applyNo::0_applyNo"})
     ...
     * 2018年7月4日 下午8:04:12
     */
    @InterfaceMonitor(errorMsg = "借据试算失败", keyParam = {"applyNo::0_applyNo"})
     ...
     */
    @InterfaceMonitor(errorMsg = "合同取消接口失败", keyParam = {"applyNo::0_applyNo"})
     ...
     * 2018/9/26 14:48
     */
    @InterfaceMonitor(errorMsg = "贷后更换还款信息失败", keyParam = {"applyNo::0_applyNo"})
