...
public class DscSyncDataApi {
     ...
     * 2018年6月4日 下午8:07:30
     */
    @InterfaceMonitor(errorMsg = "同步订单全量数据失败", keyParam = {"applyNo::0_applyNo"})
     ...
     * 2018年6月4日 下午8:07:30
     */
    @InterfaceMonitor(errorMsg = "同步请款附件信息失败", keyParam = {"applyNo::0_applyNo"})
    ...
    public InvokeResult<Boolean> syncRequestFundsAndAttachmentData(@RequestBody DscSyncDTO dscSyncDTO) {
        ...
        LOGGER.info("同步请款附件信息开始，订单编号:{}", dscSyncDTO.getApplyNo());
        result.setData(syncAlixDataService.syncRequestFundsAndAttachmentData(dscSyncDTO));
        LOGGER.info("同步请款附件信息结束，订单编号:{},返参:{}", dscSyncDTO.getApplyNo(), JSON.toJSONString(result));
        return result;
    }
     ...
     * 2018年6月4日 下午8:07:30
     */
    @InterfaceMonitor(errorMsg = "同步贷后资料接失败", keyParam = {"applyNo::0_applyNo"})
    ...
    public InvokeResult<Boolean> syncLloanData(@RequestBody DscSyncDTO param) {
        ...
        LOGGER.info("DscSyncDataApi同步贷后资料接口，订单编号:{}", applyNo);
        try {
     ...
     */
    @InterfaceMonitor(errorMsg = "规则同步失败", keyParam = {"capitalId::0_capitalId"})
     ...
     */
    @InterfaceMonitor(errorMsg = "同步易鑫放款信息失败", keyParam = {"applyNo::0_applyNo"})
     ...
     * @author sukang
     */
    @InterfaceMonitor(errorMsg = "处理合同文件异常", keyParam = {"applyNo::0_applyNo", "type::0_type"})
