...
public class PaymentCallBackStrategy {
	...
	public String execute(String callbackjson) {
		...
		try {
			...
			//============= 保存 回调报文 =============================
			this.saveAssetBankTran(paymentDto, callbackjson,null,returnJson.toJSONString(),"云南信托请款结果回调报文解析异常，返回资方结果为false");
			return returnJson.toJSONString();
		}
		...
		if(assetMainInfo == null){
			...
			//============= 保存 回调报文 =============================
			this.saveAssetBankTran(paymentDto, callbackjson,null,returnJson.toJSONString(),"云南信托请款结果回调报文通过UniqueId获取不到银行交互主表assetMainInfo，返回资方结果为false");
			return returnJson.toJSONString();
		}
		...
		if(loanInfo == null){
			...
			//============= 保存 回调报文 =============================
			this.saveAssetBankTran(paymentDto, callbackjson,assetMainInfo,returnJson.toJSONString(),"云南信托请款结果回调-向结算推送订单放款信息 ,解析云信放款信息为空，返回资方结果为false");
			return returnJson.toJSONString();
		}
		if(!loanInfo.getLoanSuccess()){ //放款没有成功
			...
			//============= 保存 回调报文 =============================
			this.saveAssetBankTran(paymentDto, callbackjson,assetMainInfo,returnJson.toJSONString(),"云南信托请款结果回调-向结算推送订单放款信息 ,解析云信放款信息放款状态为失败，返回资方结果为false");
			
			//============= 预警输出 =============================
			noticeService.noticeBzError("PaymentCallBackStrategy/execute", assetMainInfo.getApplyNo(), 
					String.format("【云南信托】订单编号：%s,放款结果失败，需要人工介入", assetMainInfo.getApplyNo()), null);
		...
		//============= 保存 回调报文 =============================
		this.saveAssetBankTran(paymentDto, callbackjson,assetMainInfo,returnJson.toJSONString(),"云南信托请款结果回调处理成功");
	...
	private void saveAssetBankTran(YTPaymentResultDTO paymentDto,String json
			,AssetMainInfo assetMainInfo,String respBody,String errorMsg){
		LOGGER.info("云南信托请款结果回调，唯一标识uniqueId：{},保存 回调报文 ",paymentDto.getUniqueId());
		AssetBankTran callbackTran = new AssetBankTran();
		callbackTran.setTranNo(paymentDto.getUniqueId()); //业务流水
		callbackTran.setReqBody(json);
		callbackTran.setApplyNo(assetMainInfo == null?null:assetMainInfo.getApplyNo());
		callbackTran.setAssetId(assetMainInfo == null?null:assetMainInfo.getId());
		callbackTran.setBankOrderNo(paymentDto.getUniqueId()); //云信唯一交互标识
		...
		callbackTran.setRepBody(respBody); //相应
		callbackTran.setApprovalCode(null); //错误码
		callbackTran.setApprovalDesc(errorMsg); //错误描述
		callbackTran.create();
	}
}
