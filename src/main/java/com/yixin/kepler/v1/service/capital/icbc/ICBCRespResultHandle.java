...
public class ICBCRespResultHandle implements BankRespResultHandle {
	...
	public BaseMsgDTO respResultHandle(BankRespResultHandleDTO resultDTO) {
		...
		try {
			...
			if (requestResult.contains(IcbcConstant.APPROVAL_CODE_SUCCESS)) {
				...
				bankRequest.setReqState(BankRequestConstant.REQ_BANK_FAIL);
				//更新数据
				bankRequest.update();
				assetMainInfo.update();
				assetBankTran.update();
			...
			bankRequest.setReqState(BankRequestConstant.RETRY_REQ_EXCEP);
			//更新数据
			bankRequest.update();
	...
	protected void asyncNotifyResult (IcbcApplyDTO<IcbcAckDataDTO> feedback){
        ...
        // 需要通过银行编号反查提报端申请编号
		AssetMainInfo mainInfo = AssetMainInfo.getByVenusApplyNo(feedback.getComm().getOrderno());
        alixDto.setApplyNo(mainInfo.getApplyNo());
	 ...
	 * @return 请求json
	 * @author YixinCapital -- chen.lin
	 *	       2018年10月23日 下午2:23:54
	 */
	public String requestBeforeHandle(AssetBankRequest task){
		logger.info("工行请求前重置请求时间开始：{}", task.getApplyNo());
		JSONObject jsonObject = JSONObject.parseObject(task.getAssetReqBody());
		if(jsonObject != null && jsonObject.containsKey("sendtime")){
			String sendTime = DateUitls.dateToStr(new Date(), "yyyyMMdd HH:mm:ss");
			jsonObject.put("sendtime", sendTime); //重新生成请求时间
			logger.info("重置请求时间成功，applyNo：{}，时间：{}", task.getApplyNo(), sendTime);
		} else{
			logger.error("重置请求时间失败，applyNo：{}", task.getApplyNo());
		}
