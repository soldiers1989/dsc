...
public class PaymentRequestStrategy extends AbstractBaseDealDomain<Object, BaseMsgDTO>{
	...
	protected InvokeResult<BaseMsgDTO> message() throws BzException {
    	...
    	if(StringUtils.isBlank(salesApplyMain.getCapitalId())){
    		throw new BzException("【云南信托请款】订单业务主表有效资方ID为空");
    	}
    	if(StringUtils.isBlank(assetMainInfo.getFinancialId())){
    		throw new BzException("【云南信托请款】银行交易主表有效资方ID为空");
    	}
    	if(!salesApplyMain.getCapitalId().equals(assetMainInfo.getFinancialId())){
    		throw new BzException("【云南信托请款】订单业务主表有效资方ID和银行交易主表有效资方ID不匹配");
    	}
		...
		try {
			//发送银行请求并获取返回结果json
			LOGGER.info("云南信托发送放款指令开始,订单编号:{},请求参数为{}",assetMainInfo.getApplyNo(),assetBankTran.getReqBody());
			result = RestTemplateUtil.sendRequest(osbUrl, assetBankTran.getReqBody(),CommonConstant.BankName.YNTRUST_BANK);
			LOGGER.info("云南信托发送放款指令结束,订单编号:{},返回结果为{}",assetMainInfo.getApplyNo(),result);
		} catch (Exception e) {
			LOGGER.error("云南信托发送放款指令异常,进行重试处理，订单编号:{}",assetMainInfo.getApplyNo(),e);
       ...
       //================================= 请款成功 =============================================================
        if(status.getIsSuccess()!=null && status.getIsSuccess()){ //请求成功
        	...
        	assetMainInfo.setAssetPhase(AssetPhaseEnum.PAYMENT_TRIAL.getPhase()); //请款阶段
	...
	private void assemblerPayment(AssetMainInfo assetMainInfo){
		...
		for(DscLoanSerialDTO serialDTO:loanSerialList){
			...
			paymentSerial.setSerialNumber(serialDTO.getBatchNo()); //银行指令包序列号 / 批次号
			paymentSerial.setSequenceNumber(serialDTO.getBusinessRefNo()); //指令包顺序号 / 业务参考号
